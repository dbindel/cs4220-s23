...
# ╔═╡ 04ec9f26-edee-11ed-2797-9989521ce513
using LinearAlgebra

# ╔═╡ 04ec9f62-edee-11ed-2c40-7d087b71d15d
using Plots

# ╔═╡ 04ec9e5e-edee-11ed-0c9e-858ad5682a0a
md"""
# Final exam
"""

# ╔═╡ 04ec9f8a-edee-11ed-09c9-5f96e5c2492f
md"""
...
# ╔═╡ 04ec9fee-edee-11ed-1cb2-3d99b7228aa6
md"""
## Snacks
...
# ╔═╡ 04ec9ff8-edee-11ed-2072-8dad3a828c10
md"""
##### Answer

"""

# ╔═╡ 04eca002-edee-11ed-3e3b-8d10c7529087
let
    xy = [1.0; 1.0]
...
# ╔═╡ 04eca0c0-edee-11ed-033e-5dbd81f8bfa7
md"""
...
4. (2 points) Consider the fixed point iteration $w_{k+1} = G(w_k)$
   where $G(w) = s(1+w)/(s+e^w)$.  The solution to $we^w = s$ is a
   fixed point of $G$ (you do not need to show this).  Argue that this
   iteration converges quadratically from good enough initial guesses.
   In fact, it converges for any initial guess in $[0,\infty)$;
   you do not have to prove this.
5. (2 points) Complete the function below to evaluate $w(s)$ and $w'(s)$
   for a given $s$.  Report values of $w(1)$ and $w'(1)$
   to at least ten digits.
"""

# ╔═╡ 04eca0e6-edee-11ed-0b8a-c382cfae9c6a
md"""
##### Answer

"""

# ╔═╡ 04eca0f2-edee-11ed-3d6c-13eb6d5a1d82
function bracket_w(s)
    # TODO: Rewrite this to avoid error for small s
    log((1+sqrt(1+4*s))/2), log(1+s)
end

# ╔═╡ 04eca0fc-edee-11ed-3140-d3a8a4a4cc02
...
# ╔═╡ 04eca124-edee-11ed-02b2-d71a4c288638
let
    s = 1.35
    h = 1e-6
    w, dw = compute_w(s)
...
# ╔═╡ 04eca17e-edee-11ed-2b8d-bdb2f95bdddb
md"""
## Quirky quadratics
...
# ╔═╡ 04eca188-edee-11ed-0a1f-afd90a1ef394
md"""
##### Answer

"""

# ╔═╡ 04eca192-edee-11ed-38f9-31262c463958
...
# ╔═╡ 04eca19c-edee-11ed-3454-c769e4391f00
let
...
# ╔═╡ 04eca25a-edee-11ed-1bf0-2f968c721847
md"""
## Block GS
...
2. (2 points) Write $R = M^{-1} N$ in terms of the component
   matrices $A, B, C, D$.
3. (3 points) Argue that $\rho(R) = \rho(D^{-1} C A^{-1} B)$ where $\rho$
   denotes the spectral radius.  You may use without argument that the
   eigenvalues of a block triangular matrix are equal to the eigenvalues
   of its diagonal blocks.
4. (3 points) Now consider the *nonlinear* block Gauss-Seidel iteration
   $Ax^{k+1} = u(y^k)$ and $D y^{k+1} = w(x^{k+1})$ where
   $u : \mathbb{R}^m \rightarrow \mathbb{R}^n$ and
   $w : \mathbb{R}^n \rightarrow \mathbb{R}^m$ satisfy
   $\|u'\|_2 \leq M_u$ and $\|w'\|_2 \leq M_w$ everywhere.
   For convenience, we can also write $y^{k+1} = D^{-1} w(A^{-1} u(y^k))$
   (we can similarly write an iteration satisfied by the $x^{k+1}$ and $x^k$
   alone). Argue that the $y_{k}$ iteration converges if
   $M_u M_w < \sigma_{min}(D) \sigma_{\min}(A)$.
"""

# ╔═╡ 04eca26e-edee-11ed-2adf-bbf7cce49180
md"""
##### Answer

"""

# ╔═╡ 04eca304-edee-11ed-1463-b7531d3effaf
md"""
## Lolling linkages
...
1. (2 points) Argue from Taylor expansion of
   ...
   constraint do we minimize the objective?
2. (4 points) Starting from an initial guess derived from the estimate
   in the previous problem, write a Newton iteration to find the optimum
   angles.  What are the angles for
   $\delta = 10^{-2}$, $\delta = 10^{-1}$, and $\delta = 0.25$?
   Give a semilog convergence plot for the $\delta = 0.25$ case.
3. (4 points) Assuming the above constrained optimization problem is
   satisfied, write a Newton solver to find $\delta$ given the
   additional equation $y = \sin(\theta_1) + \sin(\theta_2)$.
"""

# ╔═╡ 04eca30e-edee-11ed-1cd2-4fa82ffd7c82
md"""
##### Answer

"""

# ╔═╡ 04eca318-edee-11ed-1148-1960375d3480
...
# ╔═╡ 04eca324-edee-11ed-359c-415f921a66da
...
# ╔═╡ 04eca336-edee-11ed-037e-9d2050bdb76e
md"""
We provide a consistency check for part 3.
"""

# ╔═╡ 04eca340-edee-11ed-0be8-873ef307c1be
let
    δref = 0.01
...
# ╔═╡ 04eca356-edee-11ed-2aed-d7c8a8a26a9d
md"""
We also provide the code to report the numbers and plots that we want!
"""

# ╔═╡ 04eca356-edee-11ed-0434-01ee0e264c22
let
    resids = []
...
# ╔═╡ 04eca3e8-edee-11ed-1316-97a7828d6868
md"""
## Double trouble
...
3. (4 points) Write a Gauss-Newton iteration to solve the nonlinear
   ...
   unknowns in this problem are $s$, $\lambda$, and $V \in
   \mathbb{R}^{n \times 2}$; note that we have $2n+4$ equations in
   $2n+2$ unknowns.  Nevertheless, this overdetermined system is
   solvable, and Gauss-Newton should converge to it quadratically.
   Demonstrate quadratic convergence with a convergence plot.  Use an
   initial guess of $s = 0.25$, $\lambda = 3$, and $V = V_0$ computed
   by the previous equation.
"""

# ╔═╡ 04eca3fe-edee-11ed-07fc-53f4744feb49
md"""
##### Answer

"""

# ╔═╡ 04eca408-edee-11ed-172b-4dd8db63bc67
...
# ╔═╡ 04eca412-edee-11ed-2e7f-5d13cbd88c4e
...
# ╔═╡ 04eca41c-edee-11ed-0240-191c409c3ea8
...
# ╔═╡ 04eca426-edee-11ed-075a-579d6ce60eeb
...
# ╔═╡ 04eca430-edee-11ed-30eb-875a1384bd12
let
    A = [ 3.0  6.0  2.0  1.0;
          6.0  3.0  2.5  6.5;
          2.0  2.5  8.0  4.0;
