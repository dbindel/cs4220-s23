...
# ╔═╡ 2752a516-4ea1-45e3-b4b3-e5981baeec59
using Plots
...
# ╔═╡ 79d1884f-c2c5-4761-bf2e-767e7952005b
md"""
#### 1. Dear colleagues
...
# ╔═╡ aead6eba-4f6f-4631-b9f1-18af30cc9263
...
# ╔═╡ eb8af161-b41e-4d79-9060-c0260be54eaa
md"""
...
# ╔═╡ a544f873-def1-4482-9606-7c8527f64f93
# ╠═╡ disabled = true
#=╠═╡
let
	f(x) = sin(5*exp(x))
...
# ╔═╡ 03da7ac4-8455-4818-a750-04b6eaa48b65
md"""
#### 2. Twirling, twirling
...
# ╔═╡ 5588cf44-21c2-4b72-b6be-f23a9d86472b
md"""
...
# ╔═╡ c94747bf-92a3-4ba1-b864-9aee3fc8db87
function tik_bd_solve(d, e, σ, f)
	...
	for j = 1:n
		...
		# Apply second reflection to next column over, if appropriate
		if j < n
			t = 2.0*v2*enew[j]/ν
			eprev = enew[j]-t*v2
			enew[j] = -t*v1
	...
	# Solve the transformed linear system
	Bidiagonal(dnew, enew, :U)\fnew[1:n]
end

# ╔═╡ 87c4260c-6c29-4247-914c-4324d6b49fea
md"""
We can illustrate the behavior of the solver with a small example:
"""

# ╔═╡ 08a8f50c-7fc6-401d-a4b6-2e589c3cc0fc
let
	d = randn(4)
	e = randn(3)
	f = randn(4)
...
# ╔═╡ cc6b3baf-68f3-42a0-90c1-9c1f75b773bf
md"""
...
# ╔═╡ 4c3a7570-8835-49c7-b431-b64861eba95a
function bidiagonal_reduction(A)
	...
	d, e[1:n-1], mulQ, mulQT, mulP, mulPT
end

# ╔═╡ d876e245-9b40-4180-aaae-9b7260690f00
md"""
It's always good to have a sanity check; here's one for our routine above.
"""

# ╔═╡ 269bfdca-01cb-423e-9366-25548c633c29
let
	A = rand(20,10)
	x = rand(10)
...
# ╔═╡ 53bbeb6d-9c70-4d5b-91b3-8f707917d05a
md"""
Your job is to
...
# ╔═╡ 81c897b7-8099-45d0-8a7b-2499685cbada
# ╠═╡ disabled = true
#=╠═╡
let
	σ = 1e-1
...
# ╔═╡ 600556b4-bfd8-4fa2-8f12-33044a76e826
# ╠═╡ disabled = true
#=╠═╡
let
	σs = 10.0.^(0.0:-0.1:-10)
	...
	norm((rnorms-rnorms_ref)./rnorms_ref, Inf)
end
  ╠═╡ =#
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "2a3dbb426e5689c9eb57b3da420c1da1c3db9b33"
