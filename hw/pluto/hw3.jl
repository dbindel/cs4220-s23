...
Note that if $p(x) = \sum_{j=0}^d w_j T_j(x)$, then for our example we expect $p(x)-1 = w^T B^\dagger \eta$ to be a normal random variable with mean zero and mean $\|w^T B^\dagger\|$.  We can write this in Julia as `w' / B`.
"""
